...
model_mapping: Dict[str, str] = {
    ...
    "LLaMA (RU)": "IlyaGusev/saiga_llama3_8b",
    "LLaMA": "meta-llama/Llama-3.2-3B-Instruct",
    "GPT2": "openai-community/gpt2",
    "GPT-Neo": "EleutherAI/gpt-neo-125m",
    "ruGPT-3": "ai-forever/rugpt3medium_based_on_gpt2",
}
...
def register_handlers(bot: AsyncTeleBot) -> None:
    ...
    async def choose_model(message: types.Message) -> None:
        """
        Показывает клавиатуру для выбора модели.
        Аргументы:
            message (types.Message): Объект сообщения от пользователя.
        """
    ...
    async def handle_message(message: types.Message) -> None:
        ...
        # Проверяет, связано ли сообщение с учебой
        if is_educational_message(user_input):
            try:
                 # Отображаем статус "Печатает..."
                await bot.send_chat_action(message.chat.id, "typing")
                #await asyncio.sleep(1)  # Симулируем задержку в 1 секунду
